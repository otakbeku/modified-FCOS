...
class FCOS(nn.Module):
    ...
    def __init__(self, config=None, backbone='resnet50'):
        super().__init__()
        if config is None:
...
class DetectHead(nn.Module):
    ...
    def forward(self, inputs):
        ...
        reg_preds   list contains five [batch_size,4,h,w] 
        '''
        cls_logits, coords = self._reshape_cat_out(
            inputs[0], self.strides)  # [batch_size,sum(_h*_w),class_num]
        cnt_logits, _ = self._reshape_cat_out(
            inputs[1], self.strides)  # [batch_size,sum(_h*_w),1]
        reg_preds, _ = self._reshape_cat_out(
            inputs[2], self.strides)  # [batch_size,sum(_h*_w),4]
        ...
        cls_scores, cls_classes = torch.max(
            cls_preds, dim=-1)  # [batch_size,sum(_h*_w)]
        ...
        topk_ind = torch.topk(cls_scores, max_num, dim=-1,
                              largest=True, sorted=True)[1]  # [batch_size,max_num]
        _cls_scores = []
        _cls_classes = []
        _boxes = []
        for batch in range(cls_scores.shape[0]):
            ...
            _cls_classes.append(
                cls_classes[batch][topk_ind[batch]])  # [max_num]
            _boxes.append(boxes[batch][topk_ind[batch]])  # [max_num,4]
        cls_scores_topk = torch.stack(
            _cls_scores, dim=0)  # [batch_size,max_num]
        cls_classes_topk = torch.stack(
            _cls_classes, dim=0)  # [batch_size,max_num]
...
class FCOSDetector(nn.Module):
    def __init__(self, mode="training", config=None, backbone='resnet50'):
        super().__init__()
        if config is None:
            config = DefaultConfig
        self.mode = mode
        self.fcos_body = FCOS(config=config, backbone=backbone)
        if mode == "training":
